...
public class AStarAlgorithm extends JPanel {
    ...
    public void ReadFile() {
        try {
            Scanner input = new Scanner(new File("C:\\Users\\khoa.nv173196\\Desktop\\AStarAlgorithm\\src\\astaralgorithm\\map.txt"));
            row = input.nextInt();
            col = input.nextInt();
            map = new int[row][col];
            for (int i = 0; i < row; i++) {
        ...
        if (!pq.isEmpty()) {
            ...
            for (int i = 0; i < 4; i++) {
                if (isMovable(min.x + r[i], min.y + c[i])) {
                    Node child = new Node(min.x + r[i], min.y + c[i], min.level + 1, min);
    ...
    public void paintComponent(Graphics grs) {
        if (!initial) {
            ...
            Node root = new Node(11, 1, 0, null);
